...
export const register = catchAsyncError(async(req, res, next) => {
    try {
        ...
        if(registerationAttemptByUser.length >= 5) {
            return next(
                new ErrorHandler(
                    "You are exceeded the number of registration attempts. Please contact support",
                    400
                )
            )
            // if(password.length < 8 || password.length > 16) {
            //     return next(new ErrorHandler("Password must be between 8 abd 16 characters.", 400 ))
            // }
            if (password.length < 8 || password.length > 16) {
            return next(
...
export const forgotPassword = catchAsyncError(async (req, res, next) => {
    ...
    try {
        await sendEmail({email: user.email, subject: "Bookwore Library Management Password Recovery",
        message,
        })
